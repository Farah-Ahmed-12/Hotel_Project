...
if page == "Main Dashboard":
    ...
    with tab1:
        ...
        with col1:
            ...
            fig = px.pie(
                booking_status, values='count', names='status', hole=0.4,
        ...
        with col2:
            ...
            fig = px.bar(
                seg, x='marketsegment', y='bookingcount',
    ...
    with tab2:
        ...
        fig.add_trace(
            go.Scatter(x=merged['marketsegment'], y=merged['total_revenue'], name='Revenue', mode='lines+markers'),
            secondary_y=False
        )
        fig.add_trace(
            go.Scatter(x=merged['marketsegment'], y=merged['cancel_rate'], name='Cancellation %', mode='lines+markers'),
            secondary_y=True
        )
        ...
        # Funnel Analysis
        channel_grp = filtered_df.groupby('distributionchannel', observed=False).agg({
            'total_bookings': 'sum',
        ...
        fig = px.bar(
            ...
            barmode='group',
            title='Conversion / Cancel / No-show Rates'
        )
    ...
    with tab3:
        ...
        with col2:
            rev_channel = filtered_df.groupby('distributionchannel', observed=False)['total_revenue'].sum().reset_index()
            fig = px.bar(
                rev_channel,
                x='distributionchannel',
                y='total_revenue',
        ...
        rev_seg = filtered_df.groupby('marketsegment', observed=False)['total_revenue'].sum().reset_index()
        fig = px.bar(
            rev_seg,
            x='marketsegment',
            y='total_revenue',
        ...
        fig = px.scatter(
            filtered_df, x='roomnights', y='total_revenue', trendline='ols', opacity=0.4,
            title='Room Nights vs Revenue', color_discrete_sequence=['#e74c3c']
        )
        st.plotly_chart(fig, use_container_width=True)
        ...
        cohort_df['cohort'] = cohort_df['dayssincefirststay'].apply(cohort_group)
        cohort_analysis = cohort_df.groupby('cohort', observed=False).agg({
            'total_revenue': 'mean',
            'total_bookings': 'mean',
